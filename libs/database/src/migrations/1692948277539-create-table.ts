...
export class CreateTable1692948277539 implements MigrationInterface {
  name = 'CreateTable1692948277539';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE "Team" ("team_id" uuid NOT NULL DEFAULT uuid_generate_v4(), "dept_id" uuid NOT NULL, "team_nm" character varying(50) NOT NULL, "created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "updated_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "deleted_at" TIMESTAMP WITH TIME ZONE, CONSTRAINT "PK_f8ae33aeb1b677ac9f22d9bf60a" PRIMARY KEY ("team_id")); COMMENT ON COLUMN "Team"."team_id" IS '팀 UUID'; COMMENT ON COLUMN "Team"."dept_id" IS '부서 UUID'; COMMENT ON COLUMN "Team"."team_nm" IS '팀명'; COMMENT ON COLUMN "Team"."created_at" IS '생성 일시'; COMMENT ON COLUMN "Team"."updated_at" IS '수정 일시'; COMMENT ON COLUMN "Team"."deleted_at" IS '삭제 일시'`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE "MgmtItem" ("mgmt_item_id" uuid NOT NULL DEFAULT uuid_generate_v4(), "team_id" uuid NOT NULL, "mgmt_type_id" uuid NOT NULL, "partner" character varying(50) NOT NULL, "mgmt_item_nm" character varying(50) NOT NULL, "created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "updated_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "deleted_at" TIMESTAMP WITH TIME ZONE, CONSTRAINT "PK_efc0bcbbe2cd15d24bf1399ab3c" PRIMARY KEY ("mgmt_item_id")); COMMENT ON COLUMN "MgmtItem"."mgmt_item_id" IS '관리항목 UUID'; COMMENT ON COLUMN "MgmtItem"."team_id" IS '팀 UUID'; COMMENT ON COLUMN "MgmtItem"."mgmt_type_id" IS '관리유형 UUID'; COMMENT ON COLUMN "MgmtItem"."partner" IS '고객사'; COMMENT ON COLUMN "MgmtItem"."mgmt_item_nm" IS '관리항목명'; COMMENT ON COLUMN "MgmtItem"."created_at" IS '생성 일시'; COMMENT ON COLUMN "MgmtItem"."updated_at" IS '수정 일시'; COMMENT ON COLUMN "MgmtItem"."deleted_at" IS '삭제 일시'`,
    );
    await queryRunner.query(
      `CREATE TABLE "Manager" ("manager_id" uuid NOT NULL DEFAULT uuid_generate_v4(), "mgmt_item_id" uuid NOT NULL, "name" character varying(50) NOT NULL, "email" character varying(100) NOT NULL, "created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "updated_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "deleted_at" TIMESTAMP WITH TIME ZONE, CONSTRAINT "unique_manager_01" UNIQUE ("mgmt_item_id", "email", "deleted_at"), CONSTRAINT "PK_09d7ee8ca2df0e872a62b37f0b8" PRIMARY KEY ("manager_id")); COMMENT ON COLUMN "Manager"."manager_id" IS '담당자 UUID'; COMMENT ON COLUMN "Manager"."mgmt_item_id" IS '관리항목 UUID'; COMMENT ON COLUMN "Manager"."name" IS '성명'; COMMENT ON COLUMN "Manager"."email" IS 'email'; COMMENT ON COLUMN "Manager"."created_at" IS '생성 일시'; COMMENT ON COLUMN "Manager"."updated_at" IS '수정 일시'; COMMENT ON COLUMN "Manager"."deleted_at" IS '삭제 일시'`,
    );
    await queryRunner.query(
